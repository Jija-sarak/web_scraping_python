...
while n < len(top_data):
    def scrape_movie_details(movie_url):
        ...
        r = str(n+1)+top_data[n]["url"][33:]+".json"
        rd = None
        if os.path.exists(r):
            o = open(r)
            rd = o.read()
            ...
            for i in movie:
                if i == "cast":
                    ...
                    if os.path.exists(str(top_data[n]["name"].strip())+"cast.json"):
                        with open(str(top_data[n]["name"].strip())+"cast.json") as f:
                            list6 = json.load(f)
                            s = {"cast":list6}
                            ...
                            with open(str(top_data[n]["name"].strip())+"cast.json","w") as cfile:
